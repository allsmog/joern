...
def verify_prepared(root=ROOT):
    ...
    anchor_names = {'repeated_direct_include'}
    anchors = manifest.get('anchors', [])
    if (manifest.get('newProjects') != 3 or manifest.get('totalProjects') != 4
            or manifest.get('reusedProjects') != 1
            or manifest.get('sourceFiles') != 7 or len(inputs) != 7
            or len(anchors) != 1 or manifest.get('retainedAnchorCount') != 1
            or {row.get('case') for row in anchors} != anchor_names
            or manifest.get('anchorPolicy') != 'ONE_COMPLETE_REFERENCE_BYTE_IDENTICAL'
            or manifest.get('cases') != sorted({name.split('/')[0] for name in inputs})
            or len(manifest['cases']) != 4 or not anchor_names <= set(manifest['cases'])):
        raise RuntimeError('Four-case input inventory or required retained-anchor policy differs')
    for row in anchors:
        if row.get('referencePath') != 'retained-anchors/' + row['case'] + '/expected.txt':
            raise RuntimeError('Retained anchor path differs')
        require_file(root / row['referencePath'], row['expectedSha256'])
...
def verify_git_checkpoint(checkpoint):
    ...
    # Thirteenth measurements precede the atomic source/document commit.
    # Actual committed blobs and the completed closeout, not a guessed ID,
    # establish the accepted checkpoint despite deliveredCommit remaining null.
    if (source != docs or len(acceptance.get('allFrozenRustAndCargoInputs', {})) != 151
            or acceptance.get('otherFixtureFiles') != 11821
            or acceptance.get('all151FrozenBuildInputsMatchCurrentRoot') is not True
            or acceptance.get('boundedExecutionAccepted') is not True
            or acceptance.get('status') != 'BOUNDED_EXECUTION_GATES_PASSED'):
        raise RuntimeError('Acceptance does not bind this measured atomic checkpoint')
    ...
    if (gates['officialRealProjectAcceptance'] != 'passed'
            ...
            or gates['workspaceTests'] != 490 or gates['workspaceResultGroups'] != 82
            or gates['allFrozenInputFilesUnchanged'] != 151
            or gates['allFrozenFixtureFilesUnchanged'] != 11821
            or gates['nativeRelease'] != 'passed' or gates['nativeArchive'] != 'passed'):
    ...
    if (frozen_ref != acceptance['finalBuild']
            or frozen['allRustAndCargoInputs'] != acceptance['allFrozenRustAndCargoInputs']
            or gates['frozenBuildSha256'] != frozen_ref['sha256']
            or frozen['allInputsUnchangedDuringBuild'] is not True
            or frozen['testFixturesUnchangedDuringBuild'] is not True
            or len(frozen['testFixtureInputs']) != 11821):
        raise RuntimeError('Frozen build and committed acceptance disagree')
    inputs = frozen['allRustAndCargoInputs']
    if len(inputs) != 151 or not set(frozen['sources']) <= set(inputs):
    ...
    if (set(acceptance['finalBinaries']) != {'cpg', 'joern-parity'}
            or acceptance['finalBinaries'] != frozen['binaries']):
        raise RuntimeError('Frozen binary names or hashes disagree')
    for name, digest in acceptance['finalBinaries'].items():
        # Thirteenth stores name->hash; finalBuild binds their sibling bin.
        path = frozen_path.parent / name
        require_file(path, digest); external[str(path.resolve())] = digest
    # These records bind the accepted atomic commit and its final gate receipt.
    for key in ('closeout', 'gates', 'nextPreparationBinding', 'sourcePreparation'):
        row = checkpoint[key]; path = Path(row['path']).resolve()
        require_file(path, row['sha256']); external[str(path)] = row['sha256']
    closeout = json.loads(Path(checkpoint['closeout']['path']).read_bytes())
    actual_gates = json.loads(Path(checkpoint['gates']['path']).read_bytes())
    if (actual_gates != gates
            or closeout['status'] != 'LOCAL_CHECKPOINT_COMMITTED_AND_VERIFIED'
            or closeout['commit'] != source or source != docs
            or closeout['tree'] != git(repo, 'rev-parse', source + '^{tree}').decode().strip()
            or closeout['currentSourceFiles'] != 151
            or closeout['currentFixtureFiles'] != 11821
            or closeout['frozenBuild'] != frozen_ref
            or closeout['finalGates']['sha256'] != checkpoint['gates']['sha256']
            or Path(closeout['finalGates']['path']).resolve() != Path(checkpoint['gates']['path']).resolve()
            or closeout['currentSourceAndFixtureMapsEqualFrozenBuild'] is not True
            or closeout['exactCommitTreeMatchesReviewedIndex'] is not True
            or closeout['changedFiles'] != 196 or closeout['workingTreeClean'] is not True):
        raise RuntimeError('Closeout, accepted checkpoint and final gates disagree')
    # Check the actual 196-file commit against the retained precommit inventory.
    # The 11,821-file fixture verification remains the accepted closeout's work;
    # this admission does not claim another whole fixture-tree producer or audit.
    index_ref = closeout['verifiedIndex']; index_path = Path(index_ref['path']).resolve()
    require_file(index_path, index_ref['sha256']); external[str(index_path)] = index_ref['sha256']
    index = json.loads(index_path.read_bytes())
    parent = git(repo, 'rev-parse', source + '^').decode().strip()
    commit_paths = {p.decode('utf-8') for p in git(repo, 'diff', '--name-only', '-z', parent, source).split(b'\0') if p}
    if (parent != closeout['parent'] or parent != acceptance['baseCommit']
            or index['parent'] != parent or frozen['parentRevision'] != parent
            or index['stagedFileCount'] != 196 or len(index['stagedFiles']) != 196
            or commit_paths != set(index['stagedFiles'])
            or index['verifiedFrozenSourceFiles'] != 151
            or index['verifiedFrozenFixtureFiles'] != 11821):
        raise RuntimeError('Accepted commit scope differs from its reviewed index')
    for path, digest in index['stagedFiles'].items():
        if sha_bytes(git_blob(repo, source, path)) != digest:
            raise RuntimeError('Committed path differs from reviewed index: ' + path)
    bridge_ref = checkpoint['nextPreparationBinding']; bridge_path = Path(bridge_ref['path']).resolve()
    require_file(bridge_path, bridge_ref['sha256']); external[str(bridge_path)] = bridge_ref['sha256']
    bridge = json.loads(bridge_path.read_bytes())
    if (bridge['status'] != 'ACCEPTED_PARENT_BOUND_NEXT_SOURCE_PREPARATION_ONLY'
            or bridge['acceptedCommit'] != source or bridge['acceptedTree'] != closeout['tree']
            or bridge['frozenBuild'] != frozen_ref or bridge['sourceFiles'] != 151
            or bridge['fixtureFiles'] != 11821 or bridge['producerReleased'] is not False
            or bridge['inputPreparation'] != checkpoint['sourcePreparation']
            or closeout['nextPreparation'] != dict(bridge_ref, bytes=bridge_path.stat().st_size)):
        raise RuntimeError('Accepted parent/source preparation bridge differs')
    preparation_path = Path(checkpoint['sourcePreparation']['path']).resolve()
    require_file(preparation_path, checkpoint['sourcePreparation']['sha256'])
    external[str(preparation_path)] = checkpoint['sourcePreparation']['sha256']
    return {'repository': str(repo), 'sourceCommit': source, 'documentationCommit': docs,
            ...
            'frozenInputHashes': inputs, 'acceptedChangedPathHashes': index['stagedFiles'],
            'fixtureInventoryScope': 'Bound accepted build/closeout; no fresh 11821-file audit in this runner',
            'externalBindings': external, 'gitObjectAndAncestryChecksPassed': True}

def verify_checkpoint_admission(receipt, admission):
    # Pin the atomic thirteenth source/docs commit; the parent still must supply
    ...
    if not all(isinstance(receipt.get(k), str) and COMMIT.fullmatch(receipt[k])
        ...
        raise RuntimeError('Committed thirteenth source and documentation IDs are required')
    if (receipt['sourceCommit'] != admission['sourceCommit']
            or receipt['documentationCommit'] != admission['documentationCommit']):
        raise RuntimeError('Release differs from the pinned accepted thirteenth commits')
    ...
    if receipt.get('acceptance', {}).get('repositoryPath') != admission['acceptanceRepositoryPath']:
        raise RuntimeError('Release must bind the committed thirteenth acceptance')
    if not set(admission['requiredDocumentationPaths']) <= {
            row.get('repositoryPath') for row in receipt.get('documentationBlobs', [])}:
        raise RuntimeError('Committed thirteenth report and metrics bindings are required')
    ...
    if (frozen.get('sha256') != admission['frozenBuild']['sha256']
        ...
        raise RuntimeError('Release must bind the prepared final thirteenth build')
    for key in ('closeout', 'gates', 'nextPreparationBinding', 'sourcePreparation'):
        if receipt.get(key) != admission[key]:
            raise RuntimeError('Release must bind the prepared final ' + key)
    if receipt.get('acceptance') != admission['acceptance']:
        raise RuntimeError('Release acceptance hash or path differs')
    if receipt.get('documentationBlobs') != admission['documentationBlobs']:
        raise RuntimeError('Release documentation inventory differs')


def check_retained_anchors(root, selected, anchors):
    names = [row['case'] for row in anchors]
    if len(set(names)) != len(names):
        raise RuntimeError('Retained anchor names must be distinct')
    # verify_prepared separately requires the one complete repeated_direct_include anchor.
    result = {}
    for row in anchors:
...
def verify_release(path, approved_sha, prepared_sha, admission):
    ...
    if (receipt.get('schemaVersion') != 1
            or receipt.get('status') != 'PARENT_APPROVED_MACRO_BINDING_CANONICAL_PRODUCER_RELEASE'
...
def attempt(args, root=ROOT):
    ...
    # Preserve produced workspaces for a separately reviewed observer. Their
    # contents are outputs, not an unchanged-input assertion; no CPG is deleted.
    # Output inventory failure must hold promotion, never skip the final receipt.
    workspace_artifacts = {'path': str(run / 'workspace'), 'inventory': None, 'error': None}
    try:
        workspace_artifacts['inventory'] = inventory(run / 'workspace')
        observed = workspace_artifacts['inventory']
        if (observed.get('unavailable') or any(row.get('kind') == 'unavailable'
                                             for row in observed['files'].values())):
            raise RuntimeError('Workspace inventory contains unavailable observations')
    except BaseException as exc:
        workspace_artifacts['error'] = type(exc).__name__ + ': ' + str(exc)
        errors.append('workspace_inventory_failure: ' + workspace_artifacts['error'])
    ...
    receipt.update({'status': 'COMPLETE_RAW_REFERENCE_BATCH' if admitted else 'HELD_NON_REFERENCE_BATCH',
                    ...
                    **process, 'command': command, 'seconds': time.monotonic() - start,
                    ...
                    'retainedAnchorChecksApplicable': bool(manifest and manifest['anchors']),
                    'retainedAnchorByteIdentical': anchor_matches if manifest and manifest['anchors'] else None,
                    'retainedAnchors': anchor_results, 'outputs': rows,
                    'workspaceArtifacts': workspace_artifacts,
