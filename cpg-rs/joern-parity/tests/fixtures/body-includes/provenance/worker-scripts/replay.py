...
source=Path('/Users/shayaunnejad/vibe-code/.codex-worktrees/joern-oxidized-astra-body-macro-state/.local/body-include-diagnosis-v4')
refs=source/'runs/first-complete-reference-batch/expected';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
rows=[]
for case in sorted((source/'input').iterdir()):
 ...
 delta=''.join(difflib.unified_diff(expected.decode().split('\n'),actual.decode().split('\n'),fromfile='complete pinned Joern',tofile='complete candidate',lineterm='\n'))
...
(out/'replay.json').write_text(json.dumps({'status':'COMPLETE_CANDIDATE_REPLAY_DIAGNOSTICS_RETAINED','candidate':json.loads((freeze/'bindings.json').read_bytes()),'oracleReceiptSha256':sha(source/'runs/first-complete-reference-batch/run.json'),'projects':len(rows),'exact':sum(r['exact'] for r in rows),'rows':rows},indent=2)+'\n')
