...
/// source buffer. Temporary parse trees have their own bytes and never use these
...
impl BodyMacroSites {
    fn new(bytes: &[u8], initial: &MacroState) -> Self {
        Self {
            source: bytes.as_ptr() as usize,
            len: bytes.len(),
            changes: vec![(0, initial.clone())],
        }
    }
    ...
    fn record(&mut self, bytes: &[u8], offset: usize, macros: &MacroState) {
        if self.owns(bytes)
            && self.changes.last().is_none_or(|(_, prior)| !Arc::ptr_eq(prior, macros))
        {
    ...
    fn at(&self, node: Node, bytes: &[u8]) -> Option<&MacroState> {
        ...
        let index = self.changes.partition_point(|(offset, _)| *offset <= node.start_byte());
...
fn collect_type_sites(
    ...
    fn visit(
        ...
        let macros = macro_sites.and_then(|sites| sites.at(node, bytes)).unwrap_or(macros);
        if matches!(
            node.kind(),
    ...
    visit(root, bytes, &mut initial.clone(), &mut sites, macros, macro_sites);
    sites
}

impl Ctx<'_> {
    fn source_macros_at(&self, node: Node, bytes: &[u8]) -> Option<&MacroState> {
        if self.recovering_expression || self.copying_macro_argument || self.macro_expansion_code.is_some() {
            return None;
        }
    ...
    fn collect_phantoms(&mut self, body: Node, b: &[u8]) {
        ...
        collect_decl_names(body, b, &self.macros, Some(self.body_macro_sites), &mut shadowed);
    ...
    fn emit_stmt(&mut self, n: Node, b: &[u8], order: &mut i64, depth: usize) {
        let previous = self.source_macros_at(n, b).cloned()
    ...
    fn emit_expr_in_context(&mut self, n: Node, b: &[u8], depth: usize, order: i64, arg: Option<i64>) {
        if self
            .field_macro_piece
            .as_ref()
...
fn body_macro_context<'tree>(
        ...
        match node.kind() {
            ...
            _ => {
                ...
                } else if let Some((name, definition)) = source_macro_definition(node, bytes, file)
                {
                    ...
                    if let Some(body) = node.child_by_field_name("body") {
                        collect(body, bytes, file, units, visiting, once, macros, items, states,
                            header_declarations, typedefs, typedef_states, type_bindings, capture,
                            true, body_sites);
                    }
                } else if in_body {
                    for child in translation_unit_children(node, bytes) {
                        collect(child, bytes, file, units, visiting, once, macros, items, states,
                            header_declarations, typedefs, typedef_states, type_bindings, capture,
                            true, body_sites);
                    }
                }
    ...
    let mut body_macro_sites = BodyMacroSites::new(bytes, &macros);
...
fn collect_decl_names(n: Node, b: &[u8], macros: &MacroState, macro_sites: Option<&BodyMacroSites>, out: &mut Vec<String>) {
    let macros = macro_sites.and_then(|sites| sites.at(n, b)).unwrap_or(macros);
    if matches!(
        n.kind(),
