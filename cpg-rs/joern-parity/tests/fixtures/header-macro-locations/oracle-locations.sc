...
@main def exec(inputPath: String) = {
  java.nio.file.Files.list(java.nio.file.Paths.get(inputPath)).iterator.asScala.filter(java.nio.file.Files.isDirectory(_)).toList.sortBy(_.toString).foreach { fixture =>
  println("CASE|" + fixture.getFileName.toString)
  importCode(fixture.toString, fixture.getFileName.toString)
  ...
  cpg.method.sortBy(_.fullName).toList.foreach { m =>
    var index = 0
    def meta(n: AstNode): Unit = {
      val pm = n.propertiesMap.asScala
      def prop(key: String): String = Option(pm.getOrElse(key, null)).map(_.toString.replace("\n", "\\n").replace("\t", "\\t")).getOrElse("<absent>")
      println("LOC|" + List(m.fullName, index.toString, n.label, prop("NAME"), prop("CODE"), prop("ORDER"), prop("ARGUMENT_INDEX"), prop("LINE_NUMBER"), prop("COLUMN_NUMBER")).mkString("\t"))
      index += 1
      n.astChildren.toList.sortBy(_.order).foreach(meta)
    }
    meta(m)
  }
  close
