...
for n in ['package-review.json','static-boundary-checks.json','attempt-v1/verify-final.py','attempt-v1/result.json']:bind(O/n)
...
r14=j(V/'replay14.json');r4=j(B/'array-v4-replay/replay.json');cases=[];union_gain=union_loss=all_gain=all_loss=baseexact=currexact=canon=nonempty=0;identities=set()
for r in r14['rows']:
...
for name,e,prior,cur,identity in cases:
 ...
 ec_all=collections.Counter(e.split(b'\n')[:-1]);oc_all=collections.Counter(prior.split(b'\n')[:-1]);nc_all=collections.Counter(cur.split(b'\n')[:-1]);all_gain+=sum(((ec_all&nc_all)-(ec_all&oc_all)).values());all_loss+=sum(((ec_all&oc_all)-(ec_all&nc_all)).values());ec=collections.Counter(l for l in e.split(b'\n')[:-1] if l);oc=collections.Counter(l for l in prior.split(b'\n')[:-1] if l);nc=collections.Counter(l for l in cur.split(b'\n')[:-1] if l);gain=sum(((ec&nc)-(ec&oc)).values());loss=sum(((ec&oc)-(ec&nc)).values());union_gain+=gain;union_loss+=loss
...
for r in r76['rows']:
 ...
 for ver,compare in r['comparisons'].items():
  ...
  if success:
   wanted=collections.Counter(l for l in e.split(b'\n')[:-1] if l);oldc=collections.Counter(l for l in oldbytes.split(b'\n')[:-1] if l);newc=collections.Counter(l for l in cur.split(b'\n')[:-1] if l);lost=sum(((wanted&oldc)-(wanted&newc)).values());gain=sum(((wanted&newc)-(wanted&oldc)).values());ck(name+ver+' full matching counters',(lost,gain)==(compare['matchingLost'],compare['matchingGained']))
