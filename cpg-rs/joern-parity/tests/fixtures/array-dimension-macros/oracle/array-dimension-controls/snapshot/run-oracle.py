...
def verify_git_checkpoint(checkpoint):
    ...
    if (acceptance['codeRevision'] != source
            or acceptance['sourceCommitMatchesAllFrozenBuildInputs'] is not True
    ...
    if not inputs or not set(frozen['sources']) <= set(inputs):
    ...
    for name, record in acceptance['binaries'].items():
        if frozen['binaries'][name] != record['sha256']:
            raise RuntimeError('Frozen binary hashes disagree')
        path = Path(record['path'])
        path = path if path.is_absolute() else repo / repo_path(record['path'])
        require_file(path, record['sha256']); external[str(path.resolve())] = record['sha256']
...
def verify_release(path, approved_sha, prepared_sha):
    ...
    if (receipt.get('schemaVersion') != 1
            or receipt.get('status') != 'PARENT_APPROVED_BODY_INCLUDE_PRODUCER_RELEASE'
...
def attempt(args, root=ROOT):
    ...
    anchor_matches = None; external = {}; command = None; runtime_spec = None
    ...
    with (run / 'live.stdout').open('xb') as out, (run / 'live.stderr').open('xb') as err:
        try:
            ...
            checkpoint, git_before = verify_release(args.checkpoint_receipt, args.approved_receipt_sha256, prepared_sha)
            ...
            shutil.copy2(root / 'retained-anchor/expected.txt', snap / 'anchor-expected.txt')
            ...
            require_file(snap / 'anchor-expected.txt', sha(root / 'retained-anchor/expected.txt'))
    ...
    if manifest is not None:
        try:
            ...
            anchor_matches = (selected.get('body_include_macro_only') ==
                              (root / 'retained-anchor/expected.txt').read_bytes())
        except BaseException as exc:
    ...
    receipt.update({'status': 'COMPLETE_RAW_REFERENCE_BATCH' if admitted else 'HELD_NON_REFERENCE_BATCH',
                    ...
                    **process, 'command': command, 'seconds': time.monotonic() - start,
                    ...
                    'retainedAnchorByteIdentical': anchor_matches, 'outputs': rows,
