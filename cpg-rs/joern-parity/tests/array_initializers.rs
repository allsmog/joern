...
#[test]
fn ordinary_assignments_do_not_borrow_nested_declaration_locations() {
    let cpg = build("void arrays(int a) {\n a=1;\n { int a=1; }\n}\n");
    let method = cpg.method_named("arrays")[0];
    let mut assignments: Vec<_> = cpg_analysis::pass::ast_descendants(&cpg, method)
        .into_iter()
        .filter(|&node| cpg.name_of(node) == Some("<operator>.assignment"))
        .map(|node| (cpg.line_of(node), cpg.type_full_name_of(node)))
        .collect();
    assignments.sort();
    assert_eq!(
        assignments,
        [(Some(2), Some("ANY")), (Some(3), Some("void"))]
    );
}
