...
base='struct Item { int values[4]; int len; };\n'
cases={
'parameter_index':base+'#define Len values[index]\nint read(struct Item *p, int index) { return p->Len; }\n',
'global_index':base+'int index;\n#define Len values[index]\nint read(struct Item *p) { return p->Len; }\n',
'multiple_literals':base+'#define Len len + 1 + 2\nint read(struct Item *p) { return p->Len; }\n',
'expression_index':base+'#define Len values[index + 1]\nint read(struct Item *p, int index) { return p->Len; }\n',
'receiver_control':base+'#define PTR(p) (p)\nint read(struct Item *p) { return PTR(p)->len; }\n',
'multiline':base+'#define Len values[1]\nvoid use(int x);\nint read(struct Item *p) {\n  int first = p->Len;\n  use(first);\n  int second = p->Len;\n  use(second);\n  return second;\n}\n',
'field_then_index':base+'#define Values values\nint read(struct Item *p, int index) { return p->Values[index]; }\n',
'object_macro_index':base+'#define Index 1\n#define Len values[Index]\nint read(struct Item *p) { return p->Len; }\n',
}
