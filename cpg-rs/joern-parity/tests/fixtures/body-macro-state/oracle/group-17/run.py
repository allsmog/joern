...
cases={'transitive_ordinary': '#define CALL NEXT\n#define NEXT consume\n#define ARG 1\nint consume(unsigned long value);\nint sample(void) { CALL(sizeof(int[ARG]));\n#undef ARG\n#define ARG 2\nreturn ARG; }\n', 'transitive_function': '#define CALL NEXT\n#define NEXT WRAP\n#define WRAP(x) consume(x)\n#define ARG 1\nint consume(unsigned long value);\nint sample(void) { CALL(sizeof(int[ARG]));\n#undef ARG\n#define ARG 2\nreturn ARG; }\n', 'trailing_comment': '#define CALL WRAP /* trailing comment */\n#define WRAP(x) consume(x)\n#define ARG 1\nint consume(unsigned long value);\nint sample(void) { CALL(sizeof(int[ARG]));\n#undef ARG\n#define ARG 2\nreturn ARG; }\n', 'disabled_function_tail': 'int F(unsigned long value);\n#define CALL F(0)\n#define F(x) F\n#define ARG 1\nint sample(void) { CALL(sizeof(int[ARG]));\n#undef ARG\n#define ARG 2\nreturn ARG; }\n', 'object_cycle': 'int CALL(unsigned long value);\n#define CALL NEXT\n#define NEXT CALL\n#define ARG 1\nint sample(void) { CALL(sizeof(int[ARG]));\n#undef ARG\n#define ARG 2\nreturn ARG; }\n'}
...
v4=owner/'candidate-v6';baseline=root/'.local/astra-sprint/eighth-batch/final-real-differential/bin/joern-parity'
assert sha(v4/'exact.rs')=='c6fbd18f35dc536fa8ba04ba15bea12fb185745fa6e72eb1025e178597383f62'
assert sha(v4/'joern-parity')=='22d651f3daef5ce60908a34ec991d022dad009492c0c6e63e4cba1a23acd98e7'
assert sha(baseline)=='81a9d4c8c078496c2dc57b863cea8fb2a9538d72882408df08ca217dd7af6d9b'
save(p/'setup.json',{'status':'CANDIDATE_V6_BOUNDARY_REVIEW_NOT_ACCEPTANCE','time':datetime.datetime.now(datetime.timezone.utc).isoformat(),'source':bind(v4/'exact.rs'),'binary':bind(v4/'joern-parity'),'ownerReceipt':bind(v4/'bindings.json'),'buildLog':bind(owner/'build-v6.log'),'baseline':bind(baseline),'inputs':{n:bind(p/'input'/n/'main.c') for n in cases},'script':bind(p/'oracle.sc'),'traceSource':bind(peer/'cdt-trace/CdtTrace.scala'),'traceRun':bind(peer/'cdt-trace/run.json'),'upstream':bind(peer/'upstream/MacroHandler.scala')})
...
rows=[]
for variant,binary in [('accepted-eighth',baseline),('candidate-v6',v4/'joern-parity')]:
