...
def dumpProject(inputPath: String, projectName: String) = {
  importCode(inputPath, projectName)
  ...
  cpg.all.foreach { n =>
    n match {
      case a: AstNode => address(a).foreach(ad => println("LOC|" + ad + " " + a.label + " " + nprops(a, List("NAME","CODE","LINE_NUMBER","COLUMN_NUMBER"))))
      case _ =>
    }
  }
