...
(out/'replay.json').write_text(json.dumps({'status':'COMPLETE_CANDIDATE_REPLAY_DIAGNOSTICS_RETAINED','scriptSha256':sha(Path(__file__)), 'candidate':json.loads((freeze/'bindings.json').read_bytes()),'oracleReceiptSha256':sha(source/'runs/first-complete-reference-batch/run.json'),'projects':len(rows),'exact':sum(r['exact'] for r in rows),'rows':rows},indent=2)+'\n')
