...
with (P/'oracle-started.json').open('x') as marker: marker.write('{}\n')
...
with (P/'live.stdout').open('xb') as out,(P/'live.stderr').open('xb') as err:
...
sections={};n=None
for line in (P/'live.stdout').read_bytes().decode().split('\n'):
...
for n,lines in sections.items():
 ...
 q=P/'input'/n/'expected.txt';q.write_bytes(('\n'.join(lines)+'\n').encode());r['projects'].append({'case':n,'canonicalLinesIncludingSeparators':len(lines),'nonemptySelectedRecords':sum(bool(l) for l in lines),'expectedSha256':H(q)})
