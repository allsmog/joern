...
def verify_prepared(root=ROOT):
    ...
    anchor_names = {'duplicate_supplied_macro_local', 'tiny_fixedtables_include',
                    'tiny_fixedtables_inline', 'inactive_body_include'}
    anchors = manifest.get('anchors', [])
    if (manifest.get('newProjects') != 2 or manifest.get('totalProjects') != 6
            or manifest.get('reusedProjects') != 4
            or manifest.get('sourceFiles') != 11 or len(inputs) != 11
            or len(anchors) != 4 or manifest.get('retainedAnchorCount') != 4
            or {row.get('case') for row in anchors} != anchor_names
            or manifest.get('anchorPolicy') != 'ALL_FOUR_COMPLETE_REFERENCES_BYTE_IDENTICAL'
            or manifest.get('cases') != sorted({name.split('/')[0] for name in inputs})
            or len(manifest['cases']) != 6 or not anchor_names <= set(manifest['cases'])):
        raise RuntimeError('Six-case input inventory or required four-anchor policy differs')
    for row in anchors:
...
def verify_git_checkpoint(checkpoint):
    ...
    # Twelfth source/docs were committed atomically after these measurements.
    # The final closeout below proves that commit; never infer it from a parent revision.
    if (source != docs or len(acceptance.get('allRustAndCargoInputs', {})) != 149
            or acceptance.get('allFrozenFixtureFiles') != 11642
            or acceptance.get('status') != 'verified bounded C parity increment; local checkpoint not yet committed at measurement time'):
    ...
    if (gates['officialRealProjectAcceptance'] != 'passed'
            ...
            or gates['workspaceTests'] != 482 or gates['workspaceResultGroups'] != 80
            or gates['allFrozenInputFilesUnchanged'] != 149
            or gates['allFrozenFixtureFilesUnchanged'] != 11642
    ...
    if (frozen_ref != acceptance['frozenBuild']
            or frozen['allRustAndCargoInputs'] != acceptance['allRustAndCargoInputs']
    ...
    for name, digest in acceptance['binaries'].items():
        # Twelfth stores name->hash; frozenBuild binds their sibling bin.
    ...
    if (actual_gates != gates
            or closeout['status'] != 'ACCEPTED_LOCAL_CHECKPOINT'
            or closeout['commit'] != source or source != docs
            or closeout['tree'] != git(repo, 'rev-parse', source + '^{tree}').decode().strip()
            or closeout['allFrozenSourceFiles'] != 149
            or closeout['allFrozenFixtureFiles'] != 11642
            or closeout['buildBindingSha256'] != frozen_ref['sha256']
            or closeout['binaryHashes'] != frozen['binaries']
            or closeout['gatesSha256'] != checkpoint['gates']['sha256']
            or closeout['workingTreeClean'] is not True):
        raise RuntimeError('Closeout, accepted checkpoint and final gates disagree')
    for key, rel in [('acceptanceSha256', 'docs/conformance/astra-twelfth-batch-acceptance.json'),
                     ('reportSha256', 'docs/conformance/astra-twelfth-batch-2026-09-09.md'),
                     ('progressSha256', 'cpg-rs/PROGRESS.md')]:
        if blob_hashes.get(rel) != closeout[key]:
            raise RuntimeError('Closeout document binding differs: ' + key)
...
def verify_checkpoint_admission(receipt, admission):
    # Pin the atomic twelfth source/docs commit; the parent still must supply
    ...
    if not all(isinstance(receipt.get(k), str) and COMMIT.fullmatch(receipt[k])
        ...
        raise RuntimeError('Committed twelfth source and documentation IDs are required')
    if (receipt['sourceCommit'] != admission['sourceCommit']
            or receipt['documentationCommit'] != admission['documentationCommit']):
        raise RuntimeError('Release differs from the pinned accepted twelfth commits')
    ...
    if receipt.get('acceptance', {}).get('repositoryPath') != admission['acceptanceRepositoryPath']:
        raise RuntimeError('Release must bind the committed twelfth acceptance')
    if not set(admission['requiredDocumentationPaths']) <= {
            row.get('repositoryPath') for row in receipt.get('documentationBlobs', [])}:
        raise RuntimeError('Committed twelfth report and metrics bindings are required')
    ...
    if (frozen.get('sha256') != admission['frozenBuild']['sha256']
        ...
        raise RuntimeError('Release must bind the prepared final twelfth build')
    for key in ('closeout', 'gates'):
...
def check_retained_anchors(root, selected, anchors):
    ...
    # verify_prepared separately requires all four exact historical anchors.
    result = {}
    for row in anchors:
...
def verify_release(path, approved_sha, prepared_sha, admission):
    ...
    if (receipt.get('schemaVersion') != 1
            or receipt.get('status') != 'PARENT_APPROVED_INCLUDE_REFERENCE_PRODUCER_RELEASE'
