...
check('final test removes only measured nonexact case',test==oldtest.replace('        "nonprimitive_scalar_control",\n',''))
gate_names=re.findall(r'^        "([^"]+)",?$',test,re.M)
...
priorchanges=[];fails=[];priorloss=priorgain=0
check('prior instances 229',len(prior['cases'])==229)
for row in prior['cases']:
    n=row['family']+'/'+row['case'];d=P/'replay-prior-v1'/n
    for rel,h in row['files'].items():bound(d/rel,h)
    e=(d/'expected.txt').read_bytes();b=(d/'accepted-tenth.txt').read_bytes();c=(d/'candidate-v1.txt').read_bytes()
    check(n+' prior same-output flag',row['sameOutput']==(b==c))
    for v,typ in [(b,'accepted-tenth'),(c,'candidate-v1')]:
        check(n+' '+typ+' raw LF',v==lf((d/(typ+'.stdout')).read_bytes()))
        check(n+' '+typ+' exact flag',row['runs'][typ]['exact']==(row['runs'][typ]['exitCode']==0 and v==e))
    if row['successfulPair']:
        own=counters(e,b,c,True);reported={k:v for k,v in row['counts']['nonempty'].items() if k!='successfulPair'}
        check(n+' prior matching counters',own==reported);priorloss+=own['lost'];priorgain+=own['gained']
    else:fails.append({'case':n,'statuses':{k:v['exitCode'] for k,v in row['runs'].items()},'stdoutBytes':[len(b),len(c)]})
    if b!=c:priorchanges.append(n)
check('only two retained anchor instances change',priorchanges==['body-includes/tiny_fixedtables_include','body-includes/tiny_fixedtables_inline'])
check('prior matching gain14 loss0',priorgain==14 and priorloss==0)
check('one prior abort preserved',len(fails)==1 and fails[0]['statuses']=={'accepted-tenth':-6,'candidate-v1':-6} and fails[0]['stdoutBytes']==[0,0])

report={'status':'PASS_BOUNDED_FROZEN_SOURCE_AND_COMPLETED_REPLAYS','checkCount':len(checks),'checks':checks,'bindings':{str(p):sha(p) for p in [F/'bindings.json',F/'source.patch',F/'source/cpg-rs/cpg-lang-c/src/exact.rs',R/'replay.json',V/'checks.json',priorpath]},'sourceScope':'One MEMBER base-type call now uses existing TypeRole::Declaration. The renderer and nonprimitive normalize_type fallback are unchanged; CODE, suffixes, order, enum branch and clinit logic are byte-identical. No importer, solver, checker or reference changes.','totals':dict(total),'cases':rows,'priorReplay':{'instances':229,'changedInstances':priorchanges,'identicalStdoutInstances':227,'matchingGained':priorgain,'matchingLost':priorloss,'failedCases':fails},'testClassification':{'initialFrozenTestIncorrectlyIncluded':'nonprimitive_scalar_control','initialFrozenTestPreserved':True,'finalTestSha256':sha(W/'cpg-rs/joern-parity/tests/primitive_members.rs'),'fullGraphGates':18,'finalTestOnlyRemovesMeasuredDiagnosticFromExactGate':True},'limits':['Existing outputs independently recomputed; no reviewer Rust/Joern producer or Cargo execution.','The 19 complete canonical references contain 3493 LF lines and 3423 nonempty records. The replay includingSeparators counters additionally count the terminal split empty item; no gain/loss effect.','Nonprimitive scalar control retains missing external nested/tag/member scaffold, byte-identical to accepted tenth.','Prior duplicate-clinit failure remains -6/-6 with empty stdout; it is not an exact graph or successful run.','Portable package not sealed/reviewed yet. Root integration, full-project and resource acceptance remain separate.']}
