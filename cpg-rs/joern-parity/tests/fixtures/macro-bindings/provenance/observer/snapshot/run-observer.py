...
def verify_config(root=ROOT):
    ...
    for k in ['baseRunner','basePrepared','canonicalOracle','historicalObservation','sourcePreparationFreeze','acceptedCheckpointBinding']:
    ...
    require(cfg['newCases']==manifest['cases'] and len(cfg['newCases'])==4,'Four fresh cases differ')
    require(cfg['retainedCases']==[],'No historical CPG observation jobs permitted')
    require(cfg['anchorCases']==['repeated_direct_include'],'One historical supplemental anchor differs')
    ...
    checkpoint=read(cfg['acceptedCheckpointBinding']['path'])
...
def verify_observer_release(path, approved_sha, cfg, root):
    ...
    require(rel.get('schemaVersion')==1 and rel.get('status')=='PARENT_APPROVED_MACRO_BINDING_OBSERVER_RELEASE' and rel.get('observationHoldReleased') is True,'Observer hold remains active')
...
def verify_historical_anchors(cfg,base,manifest):
    ...
    for k in ['rawStdout','rawStderr']:require_binding(rec[k])
    names=[j['case'] for j in rec['jobs']];require(len(names)==len(set(names))==6,'Historical six-case inventory differs')
...
def attempt(args,root=ROOT):
    ...
    with (run/'observer.stdout').open('xb') as out,(run/'observer.stderr').open('xb') as err:
        try:
            ...
            jobs=newjobs;require(len({j['case'] for j in jobs})==4,'Four unique fresh observer jobs required')
